...
//===========================================================================
///	DoGreytoLABConversion
///
/// For whole volume. Keep it as int32. assign grey to L channel
//===========================================================================
void SLIC::DoGreytoLABConversion(
	unsigned int**&		ubuff,
	double**&					lvec,
	double**&					avec,
	double**&					bvec)
        ...
        const int(&				STEP)[3])
{
	...
	int xstrips = (0.5+double(m_width)/double(STEP[0]));
	int ystrips = (0.5+double(m_height)/double(STEP[1]));
	int zstrips = (0.5+double(m_depth)/double(STEP[2]));

    int xerr = m_width  - STEP[0]*xstrips;if(xerr < 0){xstrips--;xerr = m_width - STEP[0]*xstrips;}
    int yerr = m_height - STEP[1]*ystrips;if(yerr < 0){ystrips--;yerr = m_height- STEP[1]*ystrips;}
    int zerr = m_depth  - STEP[2]*zstrips;if(zerr < 0){zstrips--;zerr = m_depth - STEP[2]*zstrips;}
	...
	for( int z = 0; z < zstrips; z++ )
	{
		...
		for( int y = 0; y < ystrips; y++ )
		{
			...
			{
				int xe = x*xerrperstrip;
				int i = (y*STEP[1]+yoff+ye)*m_width + (x*STEP[0]+xoff+xe);
				
				kseedsl[n] = m_lvecvec[d][i];
				kseedsa[n] = m_avecvec[d][i];
				kseedsb[n] = m_bvecvec[d][i];
				kseedsx[n] = (x*STEP[0]+xoff+xe);
				kseedsy[n] = (y*STEP[1]+yoff+ye);
				kseedsz[n] = d;
				n++;
			}
		}
	...
	)
{
	...
	double invwt = 1.0/((STEP[0]/compactness)*(STEP[1]/compactness)*(STEP[2]/compactness));//compactness = 20.0 is usually good.
	...
	for( int itr = 0; itr < numIter; itr++ )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
                        y1 = max(0.0,			kseedsy[n]-offset_y);
                        y2 = min((double)m_height,	kseedsy[n]+offset_y);
                        x1 = max(0.0,			kseedsx[n]-offset_x);
                        x2 = min((double)m_width,	kseedsx[n]+offset_x);
                        z1 = max(0.0,			kseedsz[n]-offset_z);
                        z2 = min((double)m_depth,	kseedsz[n]+offset_z);
			...
			for( int z = z1; z < z2; z++ )
			{
				for( int y = y1; y < y2; y++ )
				{
					for( int x = x1; x < x2; x++ )
					{
						...
						distxyz =		spacing[0]*spacing[0]*(x - kseedsx[n])*(x - kseedsx[n]) +
										spacing[1]*spacing[1]*(y - kseedsy[n])*(y - kseedsy[n]) +
										spacing[2]*spacing[2]*(z - kseedsz[n])*(z - kseedsz[n]);
						//------------------------------------------------------------------------
						dist += distxyz*invwt;
						//------------------------------------------------------------------------
						if( dist < distvec[z][i] )
...
void SLIC::PerformSupervoxelSegmentation_VariableSandM(
    vector<double>&				kseedsl,
    vector<double>&				kseedsa,
    vector<double>&				kseedsb,
    vector<double>&				kseedsx,
    ...
    const int&					NUMITR)
{
	...
	vector<double> inv(numk, 0);//to store 1/clustersize[k] values
	vector<double> initdouble3(sz, DBL_MAX);
	vector< vector<double> > distxyz(m_depth, initdouble3);

	vector< double > initdouble(sz, DBL_MAX);
	vector< vector<double> > distvec(m_depth, initdouble);

	vector<double> initdouble2(sz, DBL_MAX); 
	vector< vector<double> > distlab(m_depth, initdouble2);

	vector<double> maxlab(numk, 10 * 10); //THIS IS THE VARIABLE VALUE OF M, just start with 10
	vector<double> maxxyz(numk, STEP * STEP); //THIS IS THE VARIABLE VALUE OF M, just start with 10
	...
	while ( numitr < NUMITR )
	{
		...
		for ( int n = 0; n < numk; n++ )
		{
			...
			int z2 = min((double)m_depth,	kseedsz[n] + offset);
			...
			for ( int z = z1; z < z2; z++ )
			{
				for ( int y = y1; y < y2; y++ )
				{
					for ( int x = x1; x < x2; x++ )
					{
						...
						double l = m_lvecvec[z][i];
						double a = m_avecvec[z][i];
						double b = m_bvecvec[z][i];

						distlab[z][i] =	(l - kseedsl[n]) * (l - kseedsl[n]) +
						                (a - kseedsa[n]) * (a - kseedsa[n]) +
						                (b - kseedsb[n]) * (b - kseedsb[n]);

						distxyz[z][i] =		(x - kseedsx[n]) * (x - kseedsx[n]) +
						                    (y - kseedsy[n]) * (y - kseedsy[n]) +
						                    (z - kseedsz[n]) * (z - kseedsz[n]);

						//------------------------------------------------------------------------
						//double dist = distlab[z][i] / maxlab[n] + distxyz[z][i] * invxywt; //only varying m, prettier superpixels
						double dist = distlab[z][i]/maxlab[n] + distxyz[z][i]/maxxyz[n];//varying both m and S
						//------------------------------------------------------------------------
		...
		for ( int d = 0; d < m_depth; d++  )
		{
			for ( int i = 0; i < sz; i++ )
			{
				if (maxlab[klabels[d][i]] < distlab[d][i]) maxlab[klabels[d][i]] = distlab[d][i];
				if (maxxyz[klabels[d][i]] < distxyz[d][i]) maxxyz[klabels[d][i]] = distxyz[d][i];
			}
		}
		//-----------------------------------------------------------------
		// Recalculate the centroid and store in the seed values
	...
	const int(&					STEP)[3])
{
	...
	int sz = width*height;
	const int SUPSZ = STEP[0]*STEP[1]*STEP[2];
    ...
    )
{
    ...
    int STEP[3];
    STEP[0] = static_cast<int>(0.5 + supervoxelsize[0]/spacing[0]);
    STEP[1] = static_cast<int>(0.5 + supervoxelsize[1]/spacing[1]);
    STEP[2] = static_cast<int>(0.5 + supervoxelsize[2]/spacing[2]);
	...
	DoGreytoLABConversion(ubuffvec, m_lvecvec, m_avecvec, m_bvecvec);

	GetKValues_LABXYZ(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, kseedsz, STEP);

	PerformSupervoxelSLIC(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, kseedsz, klabels, STEP, compactness, numIter, spacing);
	//PerformSupervoxelSegmentation_VariableSandM(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, kseedsz, klabels, STEP, numIter);
	//std::cout << "ignoring compactness" << std::endl; 
